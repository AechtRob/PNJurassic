...
public class GenLayerAddMountainsToRedwoodRare extends GenLayer
{
    ...
    public GenLayerAddMountainsToRedwoodRare(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                if (isRedwoodHills(k))
                {
                    ...
                    if (isRedwoodHills(l1) && isRedwoodHills(k2) && isRedwoodHills(j3) && isRedwoodHills(i4) && nextInt(6) == 0)
                    {
    ...
    private boolean isRedwoodHills(int biomeID) {
        if (biomeID == JURASSIC_REDWOOD_HILLS_ID) {
            return true;
        }
        return false;
    }
