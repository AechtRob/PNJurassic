...
public class GenLayerJurassicRiverBorder extends GenLayer
{
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                else if (isMire(k))
                {
                    ...
                    if ((!isMire(l1) && (!isOceanOrBeach(l1)) && (!(isMireBordered(l1))))
                            || (!isMire(k2) && (!isOceanOrBeach(k2)) && (!(isMireBordered(k2))))
                            || (!isMire(j3) && (!isOceanOrBeach(j3)) && (!(isMireBordered(j3))))
                            || (!isMire(i4) && (!isOceanOrBeach(i4)) && (!(isMireBordered(i4))))
                    ) {
                        aint1[j + i * areaWidth] = JURASSIC_RIVER_ID;
                    }
                    else
    ...
    private boolean isMireBordered(int biomeID) {
        return biomeID == JURASSIC_FLOODPLAIN_ID
                ...
                || biomeID == JURASSIC_TAIGA_FOREST_ID
